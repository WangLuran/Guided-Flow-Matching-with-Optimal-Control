...
if __name__ == '__main__':
    ...
    args.add_argument('--config', type=str, default="PepFlowww/configs/learn_angle.yaml")
    args.add_argument('--device', type=str)
    args.add_argument('--ckpt', type=str, default="rlhf_fm/peptide/dataset/model1.pt")
    args.add_argument('--output', type=str, default="PepFlowww/output")
    ...
    for i in tqdm(range(len(dataset))):
        ...
        traj_1 = model.sample(batch,num_steps=parser.num_steps,sample_bb=parser.sample_bb,sample_ang=False,sample_seq=False)
        ...
        os.makedirs(f"{parser.output}/outputs_bb",exist_ok=True)
        torch.save(traj_1[-1],f'{parser.output}/outputs_bb/{batch["id"][0]}.pt')
    dic = pd.DataFrame(dic)
    dic.to_csv(f'{parser.output}/outputs_bb.csv',index=None)