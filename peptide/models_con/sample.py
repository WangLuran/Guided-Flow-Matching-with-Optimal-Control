...
def save_samples_sc(samples,save_dir,n):
    # meta data
    batch = recursive_to(samples['batch'],'cpu')
    chain_id = [list(item) for item in zip(*batch['chain_id'])][0] # fix chain id in collate func
    icode = [' ' for _ in range(len(chain_id))] # batch icode have same problem
    ...
    for i in range(nums):
        ...
        save_pdb(data_saved,path=os.path.join(save_dir,f'sample_{n}.pdb'))
    data_saved = {
                    'chain_nb':batch['chain_nb'][0],'chain_id':chain_id,'resseq':batch['resseq'][0],'icode':icode,
                    'aa':batch['aa'][0], 'mask_heavyatom':batch['mask_heavyatom'][0], 'pos_heavyatom':batch['pos_heavyatom'][0],
                }
...
def save_samples_sc_opt(batch,save_dir,pos_new,n):
    # meta data
    ...
    data_saved = {
                    'chain_nb':batch['chain_nb'][0],'chain_id':chain_id,'resseq':batch['resseq'][0],'icode':icode,
                    'aa':batch['aa'][0], 'mask_heavyatom':batch['mask_heavyatom'][0], 'pos_heavyatom':pos_new[0],
                }
    save_pdb(data_saved,path=os.path.join(save_dir,f'opt_{n}.pdb'))
...
if __name__ == '__main__':
    ...
    args.add_argument('--SAMPLEDIR', type=str, default="PepFlowww/output/")
    parser = args.parse_args()
    SAMPLE_DIR = parser.SAMPLEDIR
    names = [n.split('.')[0] for n in os.listdir(os.path.join(SAMPLE_DIR,'outputs_bb'))]
    for name in tqdm(names):
        sample = torch.load(os.path.join(SAMPLE_DIR,'outputs_bb',f'{name}.pt'))
        os.makedirs(os.path.join(SAMPLE_DIR,'pdbs_bb',name),exist_ok=True)
        save_samples_sc(sample,os.path.join(SAMPLE_DIR,'pdbs_bb',name))